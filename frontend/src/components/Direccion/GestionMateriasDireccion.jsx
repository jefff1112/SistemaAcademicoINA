// Componente Gestión de Materias (Dirección): crea, edita, elimina y filtra materias del plan de estudio.
import React, { useState, useEffect } from 'react';
import DashboardLayout from '../Layout/DashboardLayout';
import API from '../../services/api';

// Componente principal: administra las materias con búsqueda, filtro por tipo y modal de edición.
const GestionMateriasDireccion = () => {
    // Estados: materias, especialidades, modal de edición, filtros y datos del formulario.
    ...
    // Obtiene en paralelo las materias y las especialidades desde la API.
    const cargarDatos = async () => {
        setLoading(true);
        try {
            const [materiasRes, especialidadesRes] = await Promise.all([
                // Petición GET /materias para listar las materias.
                API.get('/materias'),
                // Petición GET /especialidades para el selector del formulario.
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingMateria) {
                // Petición PUT /materias/{id} para actualizar la materia existente.
                await API.put(`/materias/${editingMateria.idMateria}`, formData);
                mostrarMensaje('Materia actualizada correctamente', 'success');
            } else {
                // Petición POST /materias para crear una nueva materia.
    ...
    const handleDelete = async (id, nombre) => {
        ...
        try {
            // Petición DELETE /materias/{id} para borrar la materia.
    ...
    // Devuelve el color que identifica el tipo de materia.
    ...
    // Filtra las materias por tipo, especialidad y por término de búsqueda.
    const materiasFiltradas = materias.filter(m => {
        if (filterTipo !== 'todos' && m.tipoMateria !== filterTipo) return false;
        if (filterEspecialidad !== 'todas' && (m.idEspecialidad ?? null) !== parseInt(filterEspecialidad)) return false;
        if (searchTerm) {
    ...
    return (
        <DashboardLayout title="Gestion de Materias - Direccion">
            {message && (
                <div style={{
                    padding: '0.75rem',
                    borderRadius: '8px',
                                    ...
                                    <tr key={m.idMateria} style={{ borderBottom: '1px solid #e5e7eb' }}>
                                        ...
                                        <td style={{ padding: '8px' }}>
                                            <span style={{
                                                ...
                                                fontSize: '12px',
                                                fontWeight: 'bold',
                                                backgroundColor: getTipoColor(m.tipoMateria),
                                                color: '#fff'
                                            }}>
                                                {getTipoLabel(m.tipoMateria)}
                                            </span>
                                        </td>
                                        <td style={{ padding: '8px' }}>{m.notaMinima}</td>
                                        <td style={{ padding: '8px' }}>{m.especialidad?.nombreEspecialidad || 'Bachillerato General'}</td>
                                        <td style={{ padding: '8px' }}>
                                            <button
                                                className="btn-edit"
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="form-group" style={{ marginTop: '12px' }}>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setFormData({ ...formData, idEspecialidad: e.target.value === '' ? null : parseInt(e.target.value) })}
                                    className="form-control"
                                    style={{ width: '100%', padding: '8px', border: '1px solid #ddd', borderRadius: '6px' }}
                                >
                                    <option value="">Bachillerato General</option>
