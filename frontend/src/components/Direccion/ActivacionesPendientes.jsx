...
const ActivacionesPendientes = () => {
    ...
    const [activeTab, setActiveTab] = useState('todos'); // 'estudiantes' | 'encargados' | 'todos'
    ...
    const cargar = async () => {
        setLoading(true);
        try {
            const data = await listarPendientes(activeTab);
            setPendientes(data || []);
        } catch (err) {
    ...
    // ============================================================
    // ACCIONES (reutilizan los mismos endpoints que ahora detectan rol)
    // ============================================================
    ...
    const handleEspera = async () => {
        ...
        try {
            ...
            mostrarMensaje(res?.mensaje || 'Marcado en espera correctamente', 'success');
            setModalEspera(null);
            setComentario('');
            cargar();
        } catch (err) {
    ...
    const getTipoBadge = (tipo) => {
        if (tipo === 'encargado') return { bg: '#e0e7ff', color: '#3730a3', label: 'Encargado' };
        return { bg: '#dcfce7', color: '#15803d', label: 'Estudiante' };
    };
    ...
    const stats = useMemo(() => ({
        ...
        estudiantes: pendientes.filter(p => p.tipo === 'estudiante').length,
        encargados: pendientes.filter(p => p.tipo === 'encargado').length
    }), [pendientes]);
    ...
    return (
        <DashboardLayout title="Activaciones Pendientes">
            <style>{`
                ...
                /* Tabs */
                .ap-tabs { display: flex; gap: 4px; margin-bottom: 16px; border-bottom: 2px solid #e2e8f0; padding-bottom: 2px; }
                .ap-tab { padding: 10px 20px; border: none; background: transparent; color: #64748b; font-size: 13px; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; margin-bottom: -2px; transition: all .2s; }
                .ap-tab:hover { color: #1e3a5f; }
                .ap-tab.active { color: #1e3a5f; border-bottom-color: #1e3a5f; background: #f8fafc; }
                ...
                .ap-stat-estudiantes { background: #e0e7ff; color: #3730a3; }
                .ap-stat-encargados { background: #fce7f3; color: #be185d; }
                ...
                .ap-tipo-badge { display: inline-block; padding: 3px 8px; border-radius: 6px; font-size: 10px; font-weight: 600; text-transform: uppercase; margin-left: 8px; }
            ...
            <div className="ap-container">
                ...
                <div className="ap-tabs">
                    <button className={`ap-tab ${activeTab === 'todos' ? 'active' : ''}`} onClick={() => setActiveTab('todos')}>
                        Todos <span style={{background:'#1e3a5f',color:'#fff',padding:'2px 6px',borderRadius:'10px',fontSize:'11px',marginLeft:'6px'}}>{stats.total}</span>
                    </button>
                    <button className={`ap-tab ${activeTab === 'estudiantes' ? 'active' : ''}`} onClick={() => setActiveTab('estudiantes')}>
                        Estudiantes <span style={{background:'#16a34a',color:'#fff',padding:'2px 6px',borderRadius:'10px',fontSize:'11px',marginLeft:'6px'}}>{stats.estudiantes}</span>
                    </button>
                    <button className={`ap-tab ${activeTab === 'encargados' ? 'active' : ''}`} onClick={() => setActiveTab('encargados')}>
                        Encargados <span style={{background:'#ec4899',color:'#fff',padding:'2px 6px',borderRadius:'10px',fontSize:'11px',marginLeft:'6px'}}>{stats.encargados}</span>
                    </button>
                </div>
                ...
                <div className="ap-stats">
                    ...
                    <div className="ap-stat ap-stat-estudiantes">
                        <span className="num">{stats.estudiantes}</span>
                        <span className="lbl">Estudiantes</span>
                    </div>
                    <div className="ap-stat ap-stat-encargados">
                        <span className="num">{stats.encargados}</span>
                        <span className="lbl">Encargados</span>
                    </div>
                </div>
                ...
                <div className="ap-card">
                    <h3>
                        {activeTab === 'estudiantes' ? 'Estudiantes' : activeTab === 'encargados' ? 'Encargados' : 'Activaciones'} con Activación Pendiente
                    ...
                    {loading ? (
                                ...
                                {searchTerm
                                    ...
                                    : activeTab === 'estudiantes'
                                        ...
                                        : activeTab === 'encargados'
                                            ? 'Todos los encargados han activado su cuenta.'
                                            : 'Todos los usuarios han activado su cuenta.'}
                            </p>
                        </div>
                    ) : (
                        <div className="table-responsive">
                            <table className="ap-tabla">
                                <thead>
                                    <tr>
                                        <th style={{ width: '220px' }}>{activeTab === 'encargados' ? 'Encargado' : 'Estudiante'}</th>
                                        <th>Correo</th>
                                ...
                                <tbody>
                                    {pendientesFiltrados.map(p => {
                                        ...
                                        const showEstudianteVinculado = p.tipo === 'encargado' && p.estudianteVinculado;
                                        return (
                                            <tr key={p.usuarioId}>
                                                <td>
                                                    ...
                                                    {showEstudianteVinculado && (
                                                        <div style={{ marginTop: '4px', fontSize: '11px', color: '#64748b' }}>
                                                            <span style={{ background: '#e0e7ff', color: '#3730a3', padding: '2px 6px', borderRadius: '4px', fontWeight: 600 }}>
                                                                {p.estudianteVinculado.nombres} {p.estudianteVinculado.apellidos}
                                                            </span>
                                                            <span style={{ marginLeft: '6px', fontFamily: 'monospace' }}>
                                                                ({p.estudianteVinculado.codigoEstudiante})
                                                            </span>
                                                        </div>
                                                    )}
                                                </td>
                                                ...
                                                <td style={{ textAlign: 'center' }}>
                                                    <span className="ap-tipo-badge" style={{ backgroundColor: tipoBadge.bg, color: tipoBadge.color }}>
                                                        {tipoBadge.label}
                                                    </span>
                                                </td>
                ...
                <div className="ap-modal-overlay" onClick={() => !saving && setModalEspera(null)}>
                    <div className="ap-modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="ap-info-box ap-info-warning">
                            Quedará marcado como <strong>pendiente de activación</strong>. Podrás reenviar el correo o activar la cuenta manualmente después.
                        </div>
                        ...
                        <textarea
                            ...
                            placeholder="Ej: No recibió el correo, se contactará por teléfono..."
                            autoFocus
                        />
                ...
                <div className="ap-modal-overlay" onClick={() => !saving && setModalPresencial(null)}>
                    <div className="ap-modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="ap-info-box ap-info-primary">
                            Establece una <strong>contraseña temporal</strong>. Podrá cambiarla al iniciar sesión por primera vez.
                        </div>
